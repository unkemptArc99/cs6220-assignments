...
- This linear behaviour of execution time can also be analyzed by the Mapper and Reducer algorithm implementation and it should be expected. The mapper goes through each word in the dataset and store it in a hash map. The complexity of the mapper algorithm is `O(nlogk)`, where the `n` is the number of words in the file, and `k` is the number of unique words. Since `k << n` (because there are not many unique words in am English book - or else there is no point in the word count program). Hence, the time complexity of the program is `O(n)`. Hence, the linear behaviour of the program compared with dataset sizes is expected.
